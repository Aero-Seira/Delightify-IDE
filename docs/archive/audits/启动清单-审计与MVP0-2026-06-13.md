...
> 完整规格已 vendored 到 **`../spec-snapshot/`**（`设计/01–10` + `例子`；只读快照，真相源仍是规划库 `设计/01`）。**本仓库的 `AGENTS.md`/`docs/`（除快照和 `current/`）/`README.md` 多为「规划/意图」，大部分未必实现——一律以代码 + 实跑结果为准。**
...
> [设计/0X] 指 `../spec-snapshot/设计/` 下对应文档。
